...
| `@samline/formatter/vanilla` | ESM + CJS dual package | Same surface as the root, with an explicit `/vanilla` subpath. Use when you want an alias that maps 1:1 to a non-framework context, or when codemods expect the convention. |
...
- A non-framework library or Node script may want the smallest, most explicit surface area — the root entrypoint is also a complete package today, but documenting the `/vanilla` alias keeps that contract explicit for future framework adapters.
...
Both entrypoints delegate to the same core module — no behavioural or size difference. Choose whichever reads better in your codebase.
...
The browser build is a single IIFE that registers a `Formatter` object on `window` (also reachable via `globalThis.Formatter`). See [docs/browser.md](browser.md) for the full surface — `format`, `regex`, `version`, plus every CDN option.
...
See [docs/typescript.md](typescript.md) for the full type table and [docs/api/regex.md](api/regex.md) for the parametric signatures.
...
| Surface | Formatter availability | `cleave-zen` | `libphonenumber-js` | `@samline/formatter` peer |
| --- | --- | --- | --- | --- |
| `@samline/formatter` (ESM / CJS) | Bundled (npm package) | bundled | bundled | n/a |
| `@samline/formatter/vanilla` (ESM / CJS) | Bundled (npm package) | bundled | bundled | n/a |
| `@samline/formatter/browser` (IIFE) | Bundled (IIFE artifact) | bundled | bundled | n/a |
...
When you pin a CDN URL, pin the same version you use in `package.json`. Mismatched `<script>` and `package.json` versions are a common source of "works on my laptop" bugs. See [docs/browser.md](browser.md) for the recommended pinning snippets.

## Related

- [docs/browser.md](browser.md) — full browser surface and CDN matrix.
- [docs/typescript.md](typescript.md) — every exported type.
- [docs/getting-started.md](getting-started.md) — install, quick start, framework-agnostic promise.
