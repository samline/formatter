...
<script>
  ...
  // Library version
  console.log(window.Formatter.version) // '2.0.3'
</script>
```
...
- `version` is injected from `package.json` at build time. Update pinned CDN
  URLs when upgrading so the loaded artifact remains deterministic.
