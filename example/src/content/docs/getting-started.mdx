...
> Pin the version in production. The browser bundle ships both `cleave-zen` and `libphonenumber-js` bundled in (~250 KB unminified). See the [Browser reference](/formatter/reference/browser/) for the full surface and CDN options.

## What's next?

<CardGrid>
	<Card title="Formatting with frameworks" icon="rocket">
		Recommended wiring for React, Vue, Svelte, vanilla, and server-side pipelines.

		[Open the guide →](/formatter/guides/formatting-with-frameworks/)
	</Card>

	<Card title="Styling and UX" icon="accessibility">
		Accessible field markup, native input attributes, CSS states, and caret considerations.

		[Styling and UX →](/formatter/guides/styling-and-ux/)
	</Card>
	...
	<Card title="Recipes" icon="puzzle">
		End-to-end recipes covering common real-world scenarios.

		[Browse recipes →](/formatter/reference/recipes/)
	</Card>
</CardGrid>
